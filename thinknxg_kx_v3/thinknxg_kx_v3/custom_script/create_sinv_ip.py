...
#             create_journal_entry_from_billing(billing["ipd_billing"])
...
#     modification_time = billing_data.get("g_modification_time", date)  # fallback if not present
...
#     if existing_je_name:
#         stored_mod_time = existing_je_name.get("custom_modification_time")
#         # If modified, cancel and remake
#         if stored_mod_time and str(stored_mod_time) != str(mod_time):
#             try:
#                 existing_je = frappe.get_doc("Journal Entry", existing_je_name)
#                 existing_je.cancel()
#                 frappe.db.commit()
#                 frappe.log(f"Cancelled JE {existing_je_name} for modified bill {bill_no}")
#             except Exception as e:
#                 frappe.log_error(f"Failed to cancel JE {existing_je_name}: {e}")
#                 return None
#         else:
#             frappe.log(f"JE for bill {bill_no} already exists and not modified. Skipping...")
#             return existing_je_name
...
#     credit_payment = next((p for p in payment_details if p["payment_mode_code"].lower() == "credit" and p["transaction_type"].lower() !="refund") , None)
...
#         if payment["payment_mode_code"].lower() == "cash" and payment["transaction_type"].lower() == "refund":
#             create_advance_refund_entry(payment, company, customer, cost_center, patient_name, bill_no, formatted_date, posting_time,customer_advance_account)

#         elif payment["payment_mode_code"].lower() == "cash" and payment["transaction_type"].lower() !="refund":
#             je_accounts.append({
...
#     for payment in payment_details:
#         if payment["payment_mode_code"].lower() in ["ip advance","uhid_advance"] and payment["transaction_type"].lower() !="refund":
#             je_accounts.append({
...
#         "company": company,
#         "user_remark": f"IP Billing for bill no {bill_no}" ,
...
# def create_advance_refund_entry(payment, company, customer, cost_center, patient_name, bill_no, formatted_date, posting_time,advance_account):
#     """
#     Create a journal entry for advance refund:
#     - Debit: Advance Received account
...
#             "company": company,
#             "user_remark": f"Advance refund for bill no {bill_no}",
#             "accounts": je_accounts
#         })
...
#         je.submit()
#         frappe.db.commit()
#         frappe.log(f"Advance refund JE created successfully: {je.name}")
#         return je.name

#     except Exception as e:
#         frappe.log_error(f"Failed to create advance refund JE: {e}")
#         return None
...
def main():
    try:
        ...
        if to_date_raw:
            ...
            t_date = getdate(add_days(nowdate(), -4))

...
#         else:
#             to_date = getdate(add_days(nowdate(), -2))
...
def fetch_advance_billing(jwt_token, from_date, to_date):
    headers_billing = {
        "Content-Type": "application/json",
        "clientCode": billing_row.get("client_code"),
        "integrationKey": billing_row.get("integration_key"),
        "x-api-key": billing_row.get("x_api_key"),
        "Authorization": f"Bearer {jwt_token}"
    }
...
def main():
    try:
        ...
        for billing in billing_data.get("jsonResponse", []):
            create_advance_refund_entry(billing.get("advance_refund"))

    except Exception as e:
        frappe.log_error(frappe.get_traceback(), "Main Error")
...
def create_advance_refund_entry(billing_data):
    try:
        ...
        # DUPLICATE CHECK
        if refund_no and frappe.db.exists("Journal Entry", {"custom_return_no": refund_no}):
            frappe.log(f"Duplicate skipped for refund_no: {refund_no}")
            return
        ...
        mode_of_payment = payment_details[0].get("payment_mode_display", "").lower()
        ...
        journal_entry = frappe.get_doc({
            ...
            "custom_bill_number": reference_no,
            "custom_return_no": refund_no,
            "custom_receipt_no": reference_no,
            "custom_patient_name": billing_data.get("patient_name"),
            "custom_uhid": billing_data.get("uhId"),
            "remark": f"Advance refund to {customer_name}",
            "custom_advance_type": billing_data.get("advance_type"),
            "custom_patient_type": billing_data.get("patient_type_code"),
        ...
        frappe.log_error(frappe.get_traceback(), "JE Creation Error")
...
def get_or_create_customer(customer_name, payer_type=None):
    ...
    existing_customer = frappe.db.exists(
        "Customer",
        {"customer_name": customer_name, "customer_group": customer_group}
    )
